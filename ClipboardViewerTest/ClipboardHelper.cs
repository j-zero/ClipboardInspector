...
namespace klemmbrett
{
    ...
    internal class ClipboardHelper
    {
        ...
        internal static byte[] GetClipboardDataBytes(IntPtr handle,string format)
        {
            ...
            while (0 != (LastRetrievedFormat = EnumClipboardFormats(LastRetrievedFormat)))
            {
                ...
                {
                    return GetClipboardDataBytes(handle,LastRetrievedFormat);
                }
            }
        ...
        */
        ...
        public static bool SetClipboardDataBytesCustomFormat(IntPtr handle, string customFormatName, byte[] data)
        {
            uint NewID = RegisterClipboardFormat(customFormatName);
            return SetClipboardDataBytes(handle, NewID, data);
        }

        public static void SetClipboardDataBytes(IntPtr handle, ClipboardFormats format, byte[] data)
        {
            SetClipboardDataBytes(handle,(uint)format, data);
        }

        public static bool SetClipboardDataBytes(IntPtr handle, uint format, byte[] data)
        {
            bool result = false;
            if (OpenClipboard(handle))
            {
                IntPtr hglobal = Marshal.AllocHGlobal(data.Length);
                GlobalLock(hglobal); // IMPORTANT!
        ...
        public static void ClearClipboard(IntPtr handle)
        {
            if (OpenClipboard(handle))
            {
                EmptyClipboard();
